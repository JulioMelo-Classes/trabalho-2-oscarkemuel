...
class Sistema {
  ...
  public:
    ...
    /*! Verifica se o usuáro está deslogado.
				@param id int com o ID do usuário
				@return um boolean, true para deslogado e false para deslogado."
		*/
    bool userNotLogged(int id);

    /*! Verifica se o servidor não existe.
				@param nome string com o nome do servidor para ser verificado
				@return um boolean, true para não existe e false para existe."
		*/
    ...
    /*! Buscar a posição do servidor escolhido pelo nome.
		...
		*/
    int serverIndexByName(string name);
